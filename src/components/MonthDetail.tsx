...
function WeatherItem({ label, value, highlight, span }: WeatherItemProps) {
  return (
    <div className={`bg-gray-800/50 rounded-lg p-2.5 ${span ? 'col-span-2 lg:col-span-1' : ''}`}>
...
export default function MonthDetail({ city, month, activeYears, planningYear, onClose }: Props) {
  ...
  const typhoonColor: Record<string, string> = {
    high: 'text-red-400', moderate: 'text-orange-400', low: 'text-yellow-400',
  };

  const overallColor =
    overall >= 7.5 ? 'text-blue-400' :
    overall >= 5.5 ? 'text-gray-100' :
    'text-red-400';
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 lg:gap-12 p-5 lg:p-0">
        ...
        {/* ── Col 1: Month + Big score ─────────────────────── */}
        <div className="mb-6 lg:mb-0">
          <div className="flex items-start justify-between mb-6 lg:mb-8">
            <div>
              <h2 className="text-2xl font-black text-white leading-tight">{MONTHS[month - 1]}</h2>
              <p className="text-xs text-gray-600 mt-0.5">{city.city} · {planningYear}</p>
            </div>
            <button
              onClick={onClose}
              aria-label="Close"
              className="text-gray-600 hover:text-white transition-colors p-1.5 rounded-lg hover:bg-gray-800 shrink-0 ml-3"
            >
          ...
          {/* Overall — large on desktop, normal on mobile */}
          <div className={`text-5xl lg:text-8xl font-black tabular-nums leading-none ${overallColor}`}>
            {overall}
          </div>
          <div className="text-[10px] text-gray-600 uppercase tracking-widest mt-2 mb-6 lg:mb-8">
            Overall score
          </div>
          ...
          <div className="flex items-start gap-6">
            <div>
              <div className="text-2xl lg:text-3xl font-black text-gray-300 tabular-nums">{comfort}</div>
              <div className="text-[10px] text-gray-600 uppercase tracking-widest mt-1.5">Comfort</div>
            </div>
            <div className="w-px self-stretch bg-gray-800" />
            <div>
              <div className="text-2xl lg:text-3xl font-black text-gray-300 tabular-nums">{crowd.toFixed(1)}</div>
              <div className="text-[10px] text-gray-600 uppercase tracking-widest mt-1.5">Crowds</div>
            </div>
          </div>
        </div>

        <div className="border-t border-gray-800/60 mb-6 lg:hidden" />
        ...
        <div className="mb-6 lg:mb-0">
          ...
          {w.notes && (
            <p className="mt-3 text-gray-600 text-xs leading-relaxed">{w.notes}</p>
          )}

          {/* Visitor trend — lives in col 2 on desktop */}
                ...
                {yearlyVisitors.map(d => (
                  <div key={d.year} className="flex items-center gap-2.5">
                    ...
                    <div className="flex-1 bg-gray-800 rounded-full h-1">
                      <div
                        className="bg-sky-500/60 h-1 rounded-full"
